  ...
  return (
    <div className="dashboard-layout">
      ...
      <div className="main-content">
        <Topbar title="Admin Dashboard" subtitle="Platform overview and controls" />
            ...
            {stats.map((s, i) => (
              <div key={i} className="stat-card">
                ...
                <div>
                  ...
                  <div className={`stat-change ${s.change.startsWith('+') ? 'up' : 'down'}`}>{s.change} this week</div>
                </div>
              </div>
            ))}
          </div>
                ...
                {MOCK_PGS.filter(p => !p.verified).slice(0, 3).map(pg => (
                  <div key={pg.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px solid var(--border)' }}>
                    ...
                    <div style={{ display: 'flex', gap: '6px' }}>
                      <button className="btn btn-sm" style={{ background: 'var(--verified-color)', color: 'white', padding: '6px 12px' }}>
                        <MdVerified size={12} /> Approve
                      </button>
                      <button className="btn btn-danger btn-sm" style={{ padding: '6px 12px' }}>Reject</button>
                    </div>
                  </div>
                ))}
              </div>
