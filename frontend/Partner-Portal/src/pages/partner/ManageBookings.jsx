...
import { MdCheck, MdClose, MdAccessTime, MdPerson, MdPhone, MdHome, MdCheckCircle } from 'react-icons/md';
                      ...
                      <tr key={b.id}>
                        <td data-label="Student">
                          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                            ...
                            <div>
                              ...
                              <div style={{ fontSize: '11px', color: '#666' }}>{b.createdAt?.toDate()?.toLocaleDateString()}</div>
                            </div>
                          </div>
                        </td>
                        <td data-label="Hostel">
                          <div style={{ textAlign: 'right' }}>
                            ...
                            <div style={{ fontSize: '11px', color: '#666' }}>{b.roomType} | ₹{b.rent}</div>
                          </div>
                        </td>
                        ...
                        <td data-label="Actions">
                          <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end', flexWrap: 'wrap' }}>
                            {b.status === 'pending' && (
                              <>
                                <button 
                                  ...
                                  style={{ background: '#E7F7EF', color: '#0D9488', padding: '6px 12px', fontSize: '12px', fontWeight: 700 }}
                                ...
                                <button 
                                  className="btn btn-sm" 
                                  style={{ background: '#FEF2F2', color: '#DC2626', padding: '6px 12px', fontSize: '12px', fontWeight: 700 }}
                            ...
                            {b.status === 'checked-in' && (
                              <span style={{ color: '#0D9488', fontSize: '13px', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '4px' }}>
                                <MdCheckCircle /> Active
                              </span>
                            )}
                            {b.status === 'cancelled' && (
                              <span style={{ color: '#DC2626', fontSize: '13px', fontWeight: 700 }}>Rejected</span>
                            )}
                          </div>
                        </td>
                      </tr>
