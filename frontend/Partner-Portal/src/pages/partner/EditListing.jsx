...
const EditListing = () => {
  ...
  return (
    <PartnerLayout title="Edit Listing">
      <button className="btn btn-ghost btn-sm" onClick={() => navigate(-1)} style={{ marginBottom: '16px' }}><MdArrowBack /> Back</button>
      <div style={{ maxWidth: '720px', background: 'white', borderRadius: '20px', border: '1px solid var(--border)', padding: '32px' }} className="p-16">
        ...
        <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end', paddingTop: '16px', borderTop: '1px solid var(--border)' }} className="flex-mobile-col">
          <button className="btn btn-ghost w-full" onClick={() => navigate(-1)}>Cancel</button>
          <button className="btn btn-primary w-full" onClick={save}><MdSave /> Save Changes</button>
        </div>
      </div>
    </PartnerLayout>
  );
